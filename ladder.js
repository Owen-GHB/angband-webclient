...
function getLatestDumps() {
  const query = "SELECT id, name, variant, level, class, race, email, username, status, date_upd, IF(date_upd = '0000-00-00', `date`, date_upd) AS sortdate, comments_count, comments_latest AS lastcom FROM ladder WHERE (date > DATE_ADD(NOW(), INTERVAL -1 WEEK) OR date_upd > DATE_ADD(NOW(), INTERVAL -1 WEEK)) AND competition = 0 ORDER BY sortdate DESC LIMIT 2";
  return connection.query(query);
}

function getLatestScreenshots() {
  const query = "SELECT id, name, variant, email, username, DATE_FORMAT(date, '%Y%m%d') AS date_f, DATE_FORMAT(date, '%H:%i') AS time_f FROM screens WHERE date > DATE_ADD(NOW(), INTERVAL -1 WEEK) ORDER BY date DESC LIMIT 2";
  return connection.query(query);
}
